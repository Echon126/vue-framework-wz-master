...
<script>
    ...
    export default {
        ...
        //将编译完成的HTMl挂载到对应虚拟DOM时触发的钩子函数
        // beforeMount(){},

        //编译好的HTMl挂载到页面完成后执行的钩子函数，此钩子函数中一般会做一些ajax请求获取数据进行数据初始化的操作，mounted在整个实例化中只执行一次
        mounted() {
            //动态添加DOM元素
        ...
        methods: {
            handleLogin() {
                this.$refs.loginForm.validate(valid => {
                    if (valid) {
                        this.loading = true;
                        console.log("登录参数-----" + this.loginForm.email + "-----"+ this.loginForm.password);
                        //dispatch  含有异步操作，向后台提交数据，写法:this.$stor.dispatch('mutations方法名',' 提交的值')
                        //commit  同步操作，用法:this.$stor.commit('mutations方法名','提交的数据')
                        this.$store.dispatch('LoginByEmail', this.loginForm).then(() => {
                            this.$Message.success('登录成功');
                            console.log("登录成功");
                            this.loading = false;

                            //在.vue模板之间跳动，在template中的写法：<router-link to ="xxxx">
                            // 编程试的跳转 this.$router.push({path:'/...'})
